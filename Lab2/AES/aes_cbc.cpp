...
int main() {
    ...
    try {
        // Get key from user
        std::string key_str;
        std::cout << "Task 2: AES CBC Encryption and Decryption without using other cryptographic external libraries\n";
        std::cout << "Enter (16/24/32)-byte key (in hex): ";
        std::getline(std::cin, key_str);
        std::vector<unsigned char> key = hex_to_bytes(key_str);

        // Get IV from user
        std::string iv_str;
        std::cout << "Enter 16-byte IV (in hex): ";
        ...
        std::cout << "Enter plaintext (supports Vietnamese): ";
        ...
        // Encrypt
        std::vector<unsigned char> ciphertext = aes.cbc_encrypt(plaintext_bytes, iv);
        std::cout << "\nCiphertext (hex): " << bytes_to_hex(ciphertext) << std::endl;

        // Decrypt
        std::vector<unsigned char> decrypted = aes.cbc_decrypt(ciphertext, iv);
